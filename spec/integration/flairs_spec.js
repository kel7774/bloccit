...
describe("routes : flairs", () => {
    beforeEach((done) => {
        ...
        sequelize.sync({force: true}).then((res) => {
            Topic.create({
                title: "Winter Games",
                description: "Post your Winter Game stories."
            })
            .then((topic) => {
                this.topic = topic;
                Flair.create({
    ...
    describe("POST /topics/:topicId/flairs/create", () => {
        it("should create a new flair and redirect", (done) => {
            const options = {
                ...
                form: {
                    name: "Very important flair",
                    color: "Magenta"
                }
            };
            request.post(options,
                (err, res, body) => {
                    Flair.findOne({where: {name: "Very important flair"}})
                    .then((flair) => {
                        expect(flair).not.toBeNull();
                        expect(flair.name).toBe("Very important flair");
    ...
    describe("GET /topics/:topicId/flairs/:id", () => {
        it("should render a view with the selected flair", (done) => {
            request.get(`${base}/${this.topic.id}/flairs/${this.flair.id}`, (err, res, body) => {
                expect(err).toBeNull();
                expect(body).toContain("Cool post flair");
                done();
            });
        });
    });
